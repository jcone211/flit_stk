...
> 最近一次全渠道实测：2026-09-02 23:5x（盘后）跑完全量 **163 项断言**（时段口径 12 + 假桥接库用例 102 + 真实实时/免费与提示词用例 27 + guard 三态用例 14 + 工具表完整性 6 等），0 失败；加 `--bridge=real` 再追加 3 条真库用例（E1 单只 30 根 0.7s / E2 十二只一次 SQL 0.3s / E3 名称解析不抽额度）。详见 §5。
...
- **≤ 7 个交易日**：上面的链整条跑，**Agent 桥接开不开都一样**——没库 / 库查询报错时不再短路，直接以空缓存走免费渠道（回归 D1 / D2 / D9 / D14 / D19）。
- **> 7 个交易日**：为保护免费渠道只读本地库（`fillKlineFromApi` 入口处 `days > 7` 闸门）；库不可用时按**根因**给 `klineDbUnavailable` 话术（「缺的是前置条件（本地库），不是接口故障」），**不再冒充「保护免费渠道」**。
...
「库拿不到」必须给用户**可行动**的结论。`resolveKlineDbPlan` / `readKlineFromDb` 返回的 `error` 键决定话术，实现在 `ai/core/ai_tools.js: klineDbUnavailable(res, days)`（四条都有回归用例：`--only=D1 / D2 / D8 / D9`）。

**仅 `days > 7` 时才会走到这四条**（≤ 7 天已经降级到免费渠道，会直接出数据）；`days` 现在会拼进首句「你查的是 N 个交易日，超过免费渠道可承担的 7 天…缺的是前置条件（本地库）」，旧版硬编码的「当前 30 日 K」与误用的「保护免费渠道」已删除。

| 触发条件 | `error` 键 | 给模型的话术 |
| --- | --- | --- |
| AI 设置没勾「Agent 桥接」 | `bridge_disabled` | 本地数据库不可用：未启用 Agent 桥接 + **启用步骤**（`node flit_bridge/server.js`、重开 AI 窗口），并说明只有超 7 天才需要库 |
| 没选主工作目录 | `workspace_not_set` | 请在 AI 窗口顶部选择工作目录（其中需有 `flit/config.json`） |
| 登记了库但读表 / 查询失败（表不存在、桥接中途挂了、SQL 被拒） | `query_failed` / `bad_table` / `bridge_unreachable` / `schema_failed` / `config_invalid` | 工作目录登记了数据库，但桥接不可用或配置未被识别：**附真实报错原文**（`取数诊断`）+「改查 7 天以内 / 修好桥接 / 联系项目作者」三条替代 |
| config 没有 `data_sources`、工作目录记忆也搜不出数据源 | `no_database` | **由于工作目录不存在可用数据库，当前无法查询该长度的 K 线，若有需求请联系项目作者** + `排查` 三条 |
