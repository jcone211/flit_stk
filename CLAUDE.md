...
- 无测试框架，但 AI 取数链路有一次性回归脚本：`node docs/verify-free-first.mjs`（116 项断言、约 25s；`--offline-cases` 只验时段口径 12 项、`--only=D6` 单跑某用例、`--bridge=real --root <含 flit/config.json 的工作目录>` 追加真库用例 E1~E3），库的分支靠 `docs/mock-bridge.mjs` 造的**假桥接**（照抄 flit_bridge 响应形状与只读闸门），不依赖用户真库；改渠道/时段/库口径后先跑它再看 Chrome（详见 `API_CHANNELS.md` §5）。
