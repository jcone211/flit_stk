...
| **数据库优先取数** | AI 取数走 **本地数据库 → 新浪/腾讯/东财免费接口 → 小石额度兜底**，盘中自动拼当日实时 bar，返回里带 `渠道` / `接口调用` / `本地库诊断` 自证（见 [API_CHANNELS.md](API_CHANNELS.md)） |
...
1. Chrome 打开 `chrome://extensions` → 开启**开发者模式** → 加载未打包的扩展程序 → 选择本项目目录
2. 顶部输入框输入股票名称/代码/URL 回车打开对应选择器的网站，可点击「一键导入」加入股票到组合
3. 已默认设置盘中3分钟刷新一次，或手动点击「开始监控」定时刷新当前选中的组合，达到阈值自动弹出系统通知
