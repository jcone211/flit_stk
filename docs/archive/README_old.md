> **ARCHIVED**（2026-09-17 归档整理）：旧版完整功能说明，只读且大量内容已过时；当前信息以根目录 `README.md` / `CLAUDE.md` 为准。
# flit stk · 量化盯盘

flit_stk 是一个轻量浏览器扩展，定时刷新股票页面，实时抓取价格数据，达到阈值时弹出系统通知。支持同花顺问财、雪球双站点，也可选择「调用 API 直取」模式不经页面、直接调用实时行情接口批量刷新。

## AI 对话分析

内置 AI 助手（无本地后端，不依赖 Open WebUI / AnythingLLM），通过插件主页工具栏「✨ AI分析」按钮打开独立对话窗口。所有大模型调用直接走 OpenAI 兼容 API（默认 DeepSeek，也支持 OpenAI / 阿里云百炼 Qwen / 本地 Ollama 等任何兼容端点），流式输出。对话中通过 **function calling 工具调用**直接读写插件数据（行情、组合、要点 / 事件、工作目录、长期记忆等），操作与手动同源（chrome.storage）、即时生效并实时同步到插件弹窗。
...
- 每份配置包含：名称、Base URL、API Key、模型；API Key 仅存本地，仅在「导出包含 API Key 的敏感数据」时随备份导出
...
| 管理要点/事件 | 创建、修改、删除交易要点与事件，读取事件记录（含筛选口径的准确率）；要点功能未开启时提示先到全局设置开启 |
| 事件超期提醒  | 事件超过一周仍未归档时提醒用户补充验证结果并归档                                                         |
...
- **安装 xiaoshi skill**：参考 **[https://www.shizixi.com/user](https://www.shizixi.com/user)** 安装小石量化 skill 到主目录的 `.claude/skills/` 下。安装后：
  ...
  - 更深层的量化分析 / 回测 / 事件时间轴等：AI 可按需阅读 skill 中的 `references/api.md` 等参考文档
- **不授权主目录**：基础能力（持仓、要点/事件、行情）仍可用，但工作目录读写、skill 感知等能力不可用
...
- **API Key**：选「小石大数据」时显示，可经 ↗ 跳转小石官网获取；仅存本地，仅在「导出包含 API Key 的敏感数据」时随备份导出
...
- **导出数据**：点击页脚导出按钮，二选一；两种导出均不携带对话中的原始图片/文件数据，以「[用户上传图片：xx.png]」「[用户上传文件：xx.pdf]」等占位文本代替：
  - **导出默认数据**：含所有组合、要点、事件、设置及 AI 对话与长期记忆；不含任何 API Key
  - **导出包含 API Key 的敏感数据**：额外含小石 / AI 接口的 API Key
- **全量恢复**：导入备份文件，覆盖恢复所有数据；默认导出不含 Key，导入后小石 / AI 接口的 Key 保持现状
...
1. **安装扩展**：Chrome 打开 `chrome://extensions` → 开启开发者模式 → 加载已解压的扩展程序 → 选择项目目录
...
- **Chrome APIs**：Alarms、Storage、Tabs、Windows、Notifications、Scripting、Offscreen（隐藏解析页）、File System Access（目录授权）
...
```text
├── background/
│   └── background.js      # Service Worker：定时刷新调度、cron、专属窗口、API 分派、数据落地（offscreen 解析）、AI 流式代理
├── ai/
...
├── popup/
│   ├── popup.js           # 弹窗主逻辑：组合管理、UI 交互、列表渲染（解析与落地在 background/offscreen）
...
├── offscreen/
│   ├── offscreen.html     # 隐藏解析页：SW 无 DOMParser，页面 HTML 解析在此执行
│   └── offscreen.js       # 接收 background 解析请求，回传解析结果
├── shared/
...
- 数据解析、落库与阈值通知在后台完成（offscreen 隐藏页解析）：弹窗关闭期间定时监控与通知照常工作（浏览器关闭则全部停止）
